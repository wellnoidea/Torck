...
function aWriting(posX, posY, string) {
	...
	this.draw = function () {
		// write something
		var x = ((aGroundThing[1].body.GetPosition().x) * SCALE - 1500 - offsetX) * 1;
		var y = ((aGroundThing[1].body.GetPosition().y) * SCALE - 150 - offsetY) * 1;
		ctx.save();
		ctx.translate(x, y);
		ctx.font = "bold 16px Arial";
		ctx.fillText(this.string, this.posX, this.posY);
		ctx.restore();
	}
}